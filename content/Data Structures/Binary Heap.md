...
The [[Binary Heap]] is a data structure that maintains a multiset $S$ of numbers by maintaining a complete binary tree where each vertex contains an element in $S$, and the element in each vertex is greater than or equal to the element in its parent, which costs a space of $\mathcal{O}(|S|)$.
