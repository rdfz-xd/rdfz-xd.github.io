...
Applying an appropriate sorting algorithm (e.g., [[Merge Sort]], [[Heapsort]]) to sort the edges and using a **Disjoint Set Union** to maintain the structure of the graph yield an algorithm that solves the problem in $\mathcal{O}(|E|\log|E|)$ time and $\mathcal{O}(|E|)$ space.
